...
public class DownloadRoatpController : Controller
{
    ...
    public DownloadRoatpController(IRoatpApiClient apiClient, ILogger<DownloadRoatpController> logger, ICsvExportService csvExportService, IRoatpApplicationApiClient applyApiClient, IMapper mapper)
    {
    ...
    public async Task<IActionResult> Download()
    {
        using (var package = new ExcelPackage())
        {
            ...
            if (registerData?.Organisations.Count > 0)
            {
            ...
            if (auditHistoryData != null && auditHistoryData.AuditRecords.Any())
            {
                IEnumerable<OrganisationAuditModel> items = auditHistoryData.AuditRecords.Select(r => (OrganisationAuditModel)r);
                auditHistoryWorksheet.Cells.LoadFromCollection(items, true);
            }
            else
            {
