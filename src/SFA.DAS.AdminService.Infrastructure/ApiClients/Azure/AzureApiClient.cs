...
namespace SFA.DAS.AdminService.Infrastructure.ApiClients.Azure
{
    public class AzureApiClient : AzureApiClientBase, IAzureApiClient
    {
        ...
        {
            _productId = azureApiClientConfiguration.ProductId;
            _groupId = azureApiClientConfiguration.GroupId;
            _requestBaseUri = new Uri(azureApiClientConfiguration.RequestBaseAddress);
        ...
        public async Task<AzureUser> CreateUser(string ukprn)
        {
            ...
            {
                throw new EntityAlreadyExistsException($"Access is already enabled.");
            }
